...
findLocalMinimums(List, Counter, [(CurrentElement, Counter)|Result]):- Counter is 0, nth0(0, List, CurrentElement), nth0(1, List, RightElement), %проверяем первый элемент в списке
                                                            CurrentElement<RightElement, 
                                                            findLocalMinimums(List, 1, Result).

findLocalMinimums(List, Counter, [(CurrentElement, Counter)|Result]):- length(List, Length), Counter is Length-1, nth0(Counter, List, CurrentElement),  %проверяем последний элементв списке
...
findLocalMinimums(List, Counter, [(CurrentElement, Counter)|Result]):- length(List, Length), Counter>0, Counter<Length-1,        % проверка всех остальных элементов кроме первого и последнего
